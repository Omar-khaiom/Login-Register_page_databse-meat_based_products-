...
def fetch_recipes_by_ingredients(ingredients_str, dietary_prefs, api_key):
    ...
    if dietary_prefs:
        params["diet"] = ",".join(dietary_prefs)
...
def generate():
    ...
    app.logger.info(f"Ingredients: {ingredients}, Dietary prefs: {dietary_prefs}")
    ...
    try:
        app.logger.info("Making API request to Spoonacular")
        ...
        user_favorites = []
        if session.get('user'):
            user_favorites = get_user_favorites(session['user'])
...
def calories():
    ...
    try:
        ...
        app.logger.error(f"Calorie lookup error: {e}")
